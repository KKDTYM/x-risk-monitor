...
DIMS = [
    ...
    ("survival", "10. 账号存续风险", 15,
     "封禁/重生史信号（复活版/重生号/被冻/被封/旧号/新号等）+4/个（上限 8）；性交易/招嫖商业信号（接线下/可约/报价/课表/口令/莞式等）+3/个（上限 7）；隐私泄露/开盒信号 +5。",
...
<div class="container">
  <div class="header">
    ...
    <p>评估日期：{esc((meta.get('evaluated_at') or '')[:10])} ｜ 引擎：v4.2（10 维度） ｜ 数据源：{esc(meta.get('data_source') or '')}</p>
  </div>
  ...
  <div class="footer">
    X 账号风险监控系统 v4.2（10 维度）｜ 数据来源：登录态 DOM 时间线 + X embed + fxTwitter ｜ 仅用于合规研究
  </div>
</div>
</body>
</html>"""
