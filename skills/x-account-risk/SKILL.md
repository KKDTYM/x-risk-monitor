...
## v5.4 微调（2026-08-13，用户反馈：日均涨粉本身不是坏事）

增长类扣分（粉丝/推文比、新号快速涨粉、日均涨粉速度）增加“低互动”闸门：仅当互动可验证且
确实低互动（中位赞 <5 或低赞占比 >70%）时扣分；互动健康不扣（真爆红新号互动不会低）；
无推文数据（互动不可验证）保留扣分（避免漏掉卖货新号）。“新号卖货人设组合”不依赖互动，保持 +5。

## v5.5 存活风险主口径（2026-08-17，用户要求：只看账号能不能活）

- 新增 `survival_score`（存活风险分，0-100）为主口径：只关注与封号真实相关的信号——
  封禁/重生史、性服务明示、商业变现、简介卖货形态、新号卖货组合、增长异常（低互动闸门）、
  单向营销号、低互动大号、擦边浓度；漏打码降为弱信号，ACC/单一 NSFW 等合规项不再主导分数。
- 口径：存活分 ≥60 高危 / 46-59 该动手 / 34-45 整改 / <34 常规关注；原合规分保留为参考。
- 回归验证：5 个有推文数据的死亡样本 4 个 ≥51（conny_vv 67 / ultimatesexy15 54 / gg64958 54 / jiajia2475 51），
  存活样本 152 个中 118 个回落到常规关注（纯成人内容但守规矩的号不再误伤）。
