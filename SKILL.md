...
## v5.0 变更（2026-08-02，10330 样本校准）

1. **关键词配置化**：所有关键词、阈值、校准数据从 `config.json` 加载，支持用户自定义
2. **维度 3/4 从分母移除**：API 自动回复 + IP 环境永远 0 分，从总分分母（142→120）移除，避免系统性评分偏低
3. **11 维度**：新增"账号存续风险"（维度 10）和"真人感/营销号形态"（维度 11）
4. **中文边界保护**：`_hit()` 函数增加 `\u4e00-\u9fa5` 边界，减少中文关键词误匹配
5. **置信度增强**：输出增加维度覆盖信息（有效维度 9/11，缺失维度标注）

## 从 v4.x 升级到 v5.0

- `config.json` 新增 `keywords`、`thresholds`、`calibration` 三个配置块
- `risk_engine.py` 启动时自动加载 `config.json`，未配置则使用硬编码默认值
- 总分分母从 142 降至 120（移除维度 3 和 4）
- 返回结果新增 `dim_coverage` 字段
...
| 10 | **账号存续风险** | 0-15 | 封禁/重生史 +4/个（上限 8）；性交易/变现信号 +3/个（上限 7）；隐私泄露/开盒 +5；幼态/未成年误判 +4；仿冒号 >=3 +2 / >=6 +4。 |
