"""
用 syndication API 抓推文列表 + Playwright 逐个访问检查"已转帖"标记
"""
import json
import re
import os
...
def fetch_syndication(username):
    ...
    if resp.status_code != 200:
        print(f"syndication HTTP {resp.status_code}")
        return None
    
    raw_html = resp.text
    # 保存原始 HTML
    save_path = f'F:/Users/Administrator/Documents/WorkBuddy/2026-07-24-21-36-14/data/syndication_raw_{username}.txt'
...
def check_retweet_with_playwright(screen_name, tweet_id):
    """用 Playwright 访问单条推文，检查是否有"已转帖"标记"""
...
def main():
    ...
    print(f"抓取 @{username} 的 syndication 数据...")
    tweets = fetch_syndication(username)
    
    if not tweets:
        print("syndication 抓取失败")
        return
    ...
    for i, tweet in enumerate(tweets):
        ...
        print(f"[{i+1}/{len(tweets)}] 检查推文 {tweet_id}...")
        ...
        if check['is_retweet']:
            retweet_count += 1
            print(f"  ** 转贴! 转 @ {check['retweet_author']}")
        else:
            original_count += 1
    ...
    output = {
        ...
        'source': 'syndication_plus_playwright_retweet_check',
