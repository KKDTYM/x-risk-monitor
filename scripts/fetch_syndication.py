"""
使用 Syndication API 抓取 X 账号推文数据
"""
import json
import os
import requests
...
def fetch_with_syndication_api(username):
    ...
    params = {
        ...
        'client_version': '2c39ee5_f7ae0aea62096f7c33a223c3b3565a4bf784555a0e40d125e29463035139',
        'withReactions': 'true',
...
def main():
    ...
    if not raw_tweets:
        # 尝试旧格式
        with open(os.path.join(DATA_DIR, f"{username}_tweets_syndication.json"), 'r', encoding='utf-8') as f:
            old_data = json.load(f)
        tweets = old_data.get('recent_tweets', [])
    else:
    ...
    profile = {
        'username': username,
        'name': '木木浆',
        'bio': '小孩魅魔🔞妈妈｜爱好meme萌物和涩涩♡',
        'followers_count': 56594,
        'following_count': 474,
        'verified': True,
    }
    ...
    # 保存
    output_path = os.path.join(DATA_DIR, f"{username}_tweets_v4.json")
    ...
    with_likes = sum(1 for t in tweets if t.get('likes', 0) > 0)
    print(f"总计 {len(tweets)} 条 | 转贴 {retweet_count} | 有互动 {with_likes}")
