...
(async () => {
  ...
  page.on('response', async (response) => {
    ...
    if (url.includes('.js') && url.includes('twimg.com')) {
      try {
        const buf = await response.body();
        const text = buf.toString('utf-8');
        const m = text.match(/"UserTweets":"([a-zA-Z0-9_-]+)"/);
        if (m) qids.add(m[1]);
      } catch (e) {}
    }
  });

  console.log('[1] loading x.com to capture JS + queryId...');
  await page.goto('https://x.com/Conny_vv', { waitUntil: 'networkidle', timeout: 40000 }).catch(()=>{});
  await delay(4000);
  const qid = [...qids][0];
  console.log('   queryId:', qid || '(none)');
  if (!qid) { console.log('[!] queryId not captured'); await browser.close(); process.exit(1); }
  ...
  const features = {
    ...
    tweet_averages_internal_enabled: false, freedom_of_speech_not_reach_fetch_enabled: true, standardized_nudges_misinfo: true,
  ...
  console.log('[2] GraphQL paging...');
  const allTweets = [];
  let cursor = null;
  for (let p = 0; p < 6; p++) {
    const result = await page.evaluate(async (qid, cursor, ct0, BEARER, features) => {
      const variables = { userId: '1549259372101918721', count: 40, cursor, includePromotedContent: false,
        ...
        if (i.type === 'TimelineAddEntries') {
          for (const e of i.content.entries) {
            if (e.entryId && e.entryId.includes('tweet')) {
              try { const t = e.content.itemContent.tweet_results.result; allTweets.push({ text: t.legacy.full_text, time: t.legacy.created_at }); added++; } catch (e2) {}
            } else if (e.content && e.content.cursorType === 'Bottom') bottom = e.content.value;
          }
        }
      }
    } catch (err) { console.log(`   parse err p${p}:`, (err.message||'').slice(0,80)); break; }
    cursor = bottom;
